...
**Google Drive** backups sit in a folder of their own in the user's Drive, under the `drive.file` scope — per-file access, so this app can only ever see files it made, and the user's own documents are invisible to it. Cashew uses the hidden app-data folder; a visible folder was chosen instead because a backup the user cannot find, copy or delete without the app is a worse backup, and because `drive.appdata` is a sensitive scope that would require a Google verification review to publish. An interval and a retained count, with old copies pruned only after a new one lands, and a list to restore, save a copy of, or delete. The scheduled run never prompts, so a lapsed permission records why it skipped rather than throwing a consent screen at somebody who has just opened their finance app. Drive is reached through its REST endpoints rather than the generated `googleapis` package.
