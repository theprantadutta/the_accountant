...
void main() {
  ...
  List<String> categoriesOf(Budget b) =>
      AppDatabase.decodeIdList(b.categoryIds);
  ...
  test(
    'a budget scoped only to the deleted wallet widens rather than breaks',
    () async {
  ...
  test(
    'an already-synced budget becomes a pending update, not a create',
    () async {
      ...
      expect(
        (await db.findBudgetById(budgetId))!.syncStatus,
  ...
  test(
    'deleting a wallet leaves category scoping alone, and vice versa',
    () async {
      final walletId = await seedWallet(db, name: 'Cash');
  ...
  test(
    'an unreadable list is treated as scoping nothing, never thrown on',
    () async {
  ...
  test(
    'merging into a category the budget already names collapses the two',
    () async {
      final survivor = await seedCategory(db, name: 'Groceries');
      final loser = await seedCategory(db, name: 'Groceries (dup)');
      ...
      expect(categoriesOf((await db.findBudgetById(budgetId))!), [
        survivor,
  ...
  test('a deleted budget is not rewritten', () async {
    final walletId = await seedWallet(db, name: 'Cash');
    final budgetId = await seedBudget(db, walletIds: [walletId]);
    await db.customStatement('UPDATE budgets SET deleted_at = 1 WHERE id = ?', [
      budgetId,
    ]);
