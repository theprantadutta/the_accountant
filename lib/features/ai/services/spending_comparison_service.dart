...
class SpendingComparisonService {
  ...
  SpendingComparison comparePeriods({
    ...
    try {
        ...
        if (transaction.type == 'income') {
          ...
          if (period1CategoryBreakdown.containsKey(transaction.categoryId)) {
            period1CategoryBreakdown[transaction.categoryId] = 
                period1CategoryBreakdown[transaction.categoryId]! + transaction.amount;
          } else {
            period1CategoryBreakdown[transaction.categoryId] = transaction.amount;
          }
        }
        ...
        if (transaction.type == 'income') {
          ...
          if (period2CategoryBreakdown.containsKey(transaction.categoryId)) {
            period2CategoryBreakdown[transaction.categoryId] = 
                period2CategoryBreakdown[transaction.categoryId]! + transaction.amount;
          } else {
            period2CategoryBreakdown[transaction.categoryId] = transaction.amount;
          }
        }
      ...
      final double incomeChangePercent = period1Income > 0 ? (incomeChange / period1Income) * 100 : 0;
      final double expenseChangePercent = period1Expenses > 0 ? (expenseChange / period1Expenses) * 100 : 0;
      final double netSavingsChangePercent = period1NetSavings != 0 ? (netSavingsChange / period1NetSavings) * 100 : 0;
      ...
      categoryComparisons.sort((a, b) => 
          (b.change.abs()).compareTo(a.change.abs()));
  ...
  String generateAIInsights(SpendingComparison comparison) {
    ...
    if (comparison.incomeChange > 0) {
      insights.write('Your income increased by \$${comparison.incomeChange.toStringAsFixed(2)} (${comparison.incomeChangePercent.toStringAsFixed(1)}%). ');
    } else if (comparison.incomeChange < 0) {
      insights.write('Your income decreased by \$${(-comparison.incomeChange).toStringAsFixed(2)} (${(-comparison.incomeChangePercent).toStringAsFixed(1)}%). ');
    } else {
    ...
    if (comparison.expenseChange > 0) {
      insights.write('Your expenses increased by \$${comparison.expenseChange.toStringAsFixed(2)} (${comparison.expenseChangePercent.toStringAsFixed(1)}%). ');
    } else if (comparison.expenseChange < 0) {
      insights.write('Your expenses decreased by \$${(-comparison.expenseChange).toStringAsFixed(2)} (${(-comparison.expenseChangePercent).toStringAsFixed(1)}%). ');
    } else {
    ...
    if (comparison.netSavingsChange > 0) {
      insights.write('This resulted in \$${comparison.netSavingsChange.toStringAsFixed(2)} (${comparison.netSavingsChangePercent.toStringAsFixed(1)}%) more savings. ');
    } else if (comparison.netSavingsChange < 0) {
      insights.write('This resulted in \$${(-comparison.netSavingsChange).toStringAsFixed(2)} (${(-comparison.netSavingsChangePercent).toStringAsFixed(1)}%) less savings. ');
    } else {
    ...
    if (comparison.categoryComparisons.isNotEmpty) {
      ...
      final topChanges = comparison.categoryComparisons.take(3).where((c) => c.change.abs() > 10).toList();
      ...
      if (topChanges.isNotEmpty) {
        ...
        for (int i = 0; i < topChanges.length; i++) {
          ...
          if (category.change > 0) {
            insights.write('${category.categoryId} (+\$${category.change.toStringAsFixed(2)})');
          } else {
            insights.write('${category.categoryId} (-\$${(-category.change).toStringAsFixed(2)})');
          }
