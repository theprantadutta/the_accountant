...
class MonthlySummaryService {
  ...
  MonthlySummary generateMonthlySummary({
    ...
    try {
      ...
      if (previousMonthExpenses > 0) {
        final double change = ((totalExpenses - previousMonthExpenses) / previousMonthExpenses) * 100;
        if (change > 5) {
      ...
      final List<CategorySpending> top3Categories = topCategories.take(3).toList();
  ...
  String generateAIInsights(MonthlySummary summary) {
    ...
    if (summary.netSavings > 0) {
      insights.write('Great job! You saved \$${summary.netSavings.toStringAsFixed(2)} this month. ');
    } else if (summary.netSavings < 0) {
      insights.write('You spent \$${(-summary.netSavings).toStringAsFixed(2)} more than you earned this month. ');
    } else {
    ...
    if (summary.spendingTrend == 'increased') {
      ...
      insights.write('Good work! Your spending decreased compared to last month. ');
    }
    ...
    if (summary.topCategories.isNotEmpty) {
      ...
      for (int i = 0; i < summary.topCategories.length && i < 3; i++) {
        ...
        insights.write('${category.categoryId} (\$${category.amount.toStringAsFixed(2)})');
    ...
    if (summary.totalIncome > 0) {
      final double savingsRate = (summary.netSavings / summary.totalIncome) * 100;
      if (savingsRate >= 20) {
        insights.write('You maintained a healthy savings rate of ${savingsRate.toStringAsFixed(1)}%. ');
      } else if (savingsRate >= 10) {
        insights.write('Your savings rate was ${savingsRate.toStringAsFixed(1)}%. ');
      } else if (savingsRate > 0) {
        insights.write('You saved ${savingsRate.toStringAsFixed(1)}% of your income. ');
      } else {
