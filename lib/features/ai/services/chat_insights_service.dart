...
class ChatInsightsService {
  ...
  String generateContextualInsights({
    ...
    try {
      ...
      final conversationContext = _analyzeConversationContext(conversationHistory);
  ...
  ConversationContext _analyzeConversationContext(List<Map<String, dynamic>> conversationHistory) {
    ...
    for (final message in recentMessages) {
      ...
      if (text.contains('category') || text.contains('food') || text.contains('transport')) {
  ...
  String _generateSpendingInsight(List<Transaction> transactions) {
    ...
    final previousCutoffDate = DateTime.now().subtract(const Duration(days: 60));
  ...
  String _generateBudgetInsight(List<Transaction> transactions, List<Budget> budgets) {
    ...
    for (final budget in budgets) {
      ...
      if (percentage >= 90) {
        budgetWarnings.add('${budget.name} is at ${percentage.toStringAsFixed(1)}% of its limit');
  ...
  String _generateSavingsInsight(List<Transaction> transactions) {
    ...
    if (totalIncome > 0) {
      final double savingsRate = ((totalIncome - totalExpenses) / totalIncome) * 100;
  ...
  List<String> generateProactiveSuggestions({
    ...
    if (budgets.isEmpty) {
      suggestions.add('Consider setting up budgets to help manage your spending.');
    }
    ...
    if (recentExpenses.length > 10) {
      suggestions.add('Review your recent spending to identify areas where you could cut back.');
    }
    ...
    if (totalIncome > 0) {
      ...
      if (savingsRate < 15) {
        suggestions.add('Try to increase your savings rate. Even 1% more can make a difference over time.');
      }
    }
